...
const styleOptions: StyleOption[] = [
  ...
  { id: 'neu', title: '뉴모피즘', tagline: '표면에서 돋아난 듯한 버튼', lines: [
    '배경과 카드가 같은 연회색(#e4e9f2 계열)인 단일 톤 화면으로 만든다.',
    '카드와 버튼에 밝은 그림자(-6px -6px 흰색)와 어두운 그림자(6px 6px 회청색)를 함께 줘서 표면에서 돋아난 입체감을 만든다.',
    '누르는 순간(active)에는 inset 그림자로 반전시켜 눌린 느낌을 준다.',
    '보더 없이 그림자만으로 요소를 구분하고, 포인트 색은 한 가지만 절제해서 쓴다.',
    '텍스트는 진회색으로 부드럽게 유지하고 대비가 부족한 곳은 굵기로 보완한다.',
  ] },
  { id: 'editorial', title: '에디토리얼 매거진', tagline: '잡지 지면 같은 타이포 중심', lines: [
    '종이 느낌의 밝은 배경(#faf8f4)에 검정 텍스트로 잡지 지면처럼 만든다.',
    '제목은 큰 세리프(명조) 시스템 폰트, 본문은 산세리프로 서체 대비를 준다.',
    '섹션 사이를 두꺼운 검정 룰(2~3px)과 얇은 헤어라인으로 나눠 신문 레이아웃 느낌을 낸다.',
    '첫 문단의 첫 글자는 드롭캡으로 키우고, 라벨은 자간 넓은 대문자 소형 타이포로 처리한다.',
    '장식은 흑백 인라인 SVG로 절제해서 쓰고 컬러는 포인트 한 곳에만 허용한다.',
  ] },
  { id: 'aurora', title: '오로라 그라데이션', tagline: '산뜻한 SaaS 랜딩 무드', lines: [
    '배경에 파스텔 오로라 그라데이션(하늘색→라벤더→핑크)을 부드럽게 깐다.',
    '콘텐츠 카드는 순백색에 18~22px 라운드, 은은한 컬러 그림자로 배경 위에 띄운다.',
    '제목이나 핵심 수치에 그라데이션 텍스트(background-clip: text)를 포인트로 쓴다.',
    '주요 버튼은 그라데이션 배경에 흰 글자, hover 시 살짝 밝아지게 한다.',
    '전체적으로 밝고 산뜻한 SaaS 랜딩 페이지 무드를 유지한다.',
  ] },
];
