...
import { memoryPages } from '@/lib/page-store';
...
export async function GET(_request: Request, context: { params: Promise<{ pageId: string }> }) {
  ...
  if (memoryPages.has(pageId)) {
    ...
    if (page.expiresAt <= now) { memoryPages.delete(pageId); return expired(); }
    html = page.html;
  } else {
    const db = env.DB;
    const files = env.FILES;
    if (!db || !files) return expired();
